...
constexpr float TURN_SPEED = XM_PIDIV2 * 0.005f;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
	{
		...
		float x = move.x * -cosf(g_yaw) - move.z * sinf(g_yaw);
		float z = move.x * sinf(g_yaw) - move.z * cosf(g_yaw);
		g_f3_eye_pos.x += x * MOVE_SPEED;
		g_f3_eye_pos.z += z * MOVE_SPEED;

		// Determine the look direction.
		float r = cosf(g_pitch);
		g_f3_lock_at.x = r * sinf(g_yaw);
		g_f3_lock_at.y = sinf(g_pitch);
		g_f3_lock_at.z = r * cosf(g_yaw);
